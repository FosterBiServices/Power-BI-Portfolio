# Builds a single-file, windowed AboutThisReportGenerator.exe into .\dist
...
if (-not (Test-Path ".\about_report_generator\app.py")) {
  throw "about_report_generator\app.py was not found. Run this script from the project root."
}
...
Invoke-Native $VenvPython @("-m", "pip", "install", "PySide6>=6.8,<6.10", "pyinstaller")

$Version = (& $VenvPython -c "import tomllib; print(tomllib.load(open('pyproject.toml', 'rb'))['project']['version'])").Trim()
...
Invoke-Native $VenvPython @(
  ...
  "--noconfirm", "--clean",
  "--onefile", "--windowed",
  "--name", "AboutThisReportGenerator",
  "--specpath", "build",
  "--exclude-module", "tkinter",
  "launcher.py"
)

$ExePath = Join-Path $ProjectRoot "dist\AboutThisReportGenerator.exe"
...
Write-Host ""
Write-Host "About This Report Generator v$Version built ($SizeMb MB):" -ForegroundColor Green
Write-Host $ExePath
Write-Host "Single file - share AboutThisReportGenerator.exe on its own."
